...
def prepare_targets_for_loss(coordinate_tl_list, dataset, num_particles):
    targets = []
    for index, coordinate_tl in enumerate(coordinate_tl_list):
        ...
        boxes = boxes / dataset.sub_micrograph_size  # Only works if sub micrograph is square
        ...
        pad_size = num_particles - boxes.shape[0]
        if pad_size > 0:
        ...
        label_pad_size = num_particles - labels.shape[0]
        if label_pad_size > 0:
