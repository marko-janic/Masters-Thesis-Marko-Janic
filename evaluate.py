...
def generate_heatmaps_volume(dataset, vit_model, vit_image_processor, device, latent_dim, model,
                             vit_input_size: int, use_fbp: bool):
    """
    ...
    for z in tqdm(range(160, 348), desc="Generating heatmaps volume"):
        z_slice = volume[z]
...
def evaluate(args, model, vit_model, vit_image_processor, dataset, test_dataloader, criterion, example_predictions):
    ...
    with (torch.no_grad()):
        ...
        if not args.volume_evaluation:
            ...
            coordinates = torch.from_numpy(peak_local_max(output_heatmaps_volume_numpy, min_distance=8,
                                                          threshold_abs=0.7))
